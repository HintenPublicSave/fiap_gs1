...
Ao mesmo tempo, órgãos de resposta como a **Defesa Civil e o Corpo de Bombeiros enfrentam limitações logísticas, escassez de recursos e déficit de pessoal**, especialmente em áreas urbanas densas. Nessa situação, a comunicação emergencial — essencial para orientar evacuações preventivas — torna-se vulnerável, pois **informações claras e precisas podem reduzir significativamente o número de vítimas**. A rápida propagação de notícias verdadeiras e falsas em redes sociais torna ainda mais urgente dispor de meios confiáveis e ágeis de divulgação.

Diante desse contexto, desenvolvemos um **agente de inteligência artificial** que incorpora **um modelo de previsão de enchentes** treinado com dados históricos e leituras de sensores, além de **gerar automaticamente conteúdos informativos e alertas**. O objetivo é **otimizar tempo e recursos das equipes de emergência**, garantindo que orientações essenciais cheguem de forma rápida e eficaz à população em risco. Adicionalmente, propomos a **criação de um aplicativo móvel** que incluirá funcionalidades de suporte a buscas e resgates em situações de desastre**, como **monitoramento de áreas de risco**, **detecção de perda de sinal suspeita** e **notificação automática de autoridades**.
...
- **Criação de imagens ilustrativas**: Para reforçar o **impacto visual**, o sistema gera **gráficos, mapas e ilustrações** que permitem à população **compreender rapidamente a gravidade da situação** e as ações recomendadas.
...
Dessa forma, o agente oferece um **conjunto de ferramentas** que aprimoram significativamente a **capacidade de resposta das equipes de emergência**, favorecendo **decisões mais rápidas e assertivas em situações críticas**.

## ⚙️ Treinamento do Modelo de Previsão
...
Também armazenamos o modelo em si em **modelos_treinados[nome]** e as predições em **y_preds[nome]**. Registramos o **tempo de treinamento** em uma lista separada, construindo posteriormente o **DataFrame df_tempos** com cada par (**nome do modelo**, **tempo em segundos**).
...
## 🚀 Próximos Passos e Visão Futura: Expansão para Aplicativo Móvel
...
- **Base de dados reais:** Utilização de informações da plataforma [disasterscharter.org](https://disasterscharter.org), que reúne imagens de satélite e relatórios globais sobre desastres.
...
(Adicionar aqui o README específico do circuito de sensores) <----------------------------------------------------------------------------------------------------------
...
- As chaves de API utilizadas neste projeto foram enviadas exclusivamente ao orientador via Teams.
...
- <b>src</b>: Diretório principal que contém todo o código-fonte desenvolvido ao longo das fases do projeto. Ele está organizado nos seguintes submódulos:
  ...
  - <b>logger</b>: Código responsável por registrar (logar) todas as operações executadas no banco de dados, garantindo rastreabilidade.
  - <b>service</b>: Responsável pela conexão com a API pública de previsão do tempo, realizando a coleta de dados meteorológicos externos.
